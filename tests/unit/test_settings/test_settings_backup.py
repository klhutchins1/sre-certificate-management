...
def test_backup_with_special_characters(test_env):
    ...
    test_files = [
        ...
        # Windows does not allow some special characters in filenames
        "file_with_!@#$%^&().txt"
    ]
    # Only add unicode/special char file if not on Windows
    if not sys.platform.startswith('win'):
        test_files.append("file_with_unicode_测试.txt")
    ...
    for file_path in special_dir.glob("*"):
        file_path.unlink()
    special_dir.rmdir()
    # latest_backup.unlink()  # Removed, already deleted above
    # manifest_files[0].unlink()  # Removed, already deleted above
