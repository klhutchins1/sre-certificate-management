...
- **Scanning & Monitoring:** Internal/external scans, SANs, scan history, alerts, certificate chain validation, and scan control (pause/stop/resume).
...
- **Scanning Certificates:** Use the "Scan" sidebar, enter hostnames, start scan, view results/errors.
  ...
  - Pause, resume, or stop ongoing scans using the scan controls.
...
## 10. Proxy Certificate Deduplication

The system automatically detects and merges duplicate proxy certificates during scanning. Proxy certificates are dynamically generated by corporate proxies/TLS interception devices, creating multiple certificates with different serial numbers but identical logical identity.

### Features

- **Automatic Detection:** Identifies proxy certificates by CA issuer (not serial numbers)
- **Intelligent Merging:** Combines duplicates based on logical identity (CA + target + expiration)
- **Data Preservation:** Maintains all related data (bindings, scans, tracking)
- **Manual Tools:** Scripts available for cleaning existing duplicates

### Manual Deduplication

For cleaning existing duplicate proxy certificates:
...
For detailed information, see [docs/deduplication.md](docs/deduplication.md).
...
- **[Functional Specification](docs/functional_spec.md)** - Feature specifications
- **[Migration Guide](docs/MIGRATION_GUIDE.md)** - Database migration instructions
- **[Testing Strategy](docs/TESTING_STRATEGY.md)** - Testing guidelines and best practices
- **[Deduplication Guide](docs/deduplication.md)** - Certificate deduplication documentation
- **[Missing Features Plan](docs/MISSING_FEATURES_PLAN.md)** - Roadmap for future features
- **[Security Template](docs/SECURITY_TEMPLATE.md)** - Security best practices
- **[Test Isolation Guide](docs/TEST_ISOLATION_GUIDE.md)** - Test isolation documentation
